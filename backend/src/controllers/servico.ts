...
import { Servicos } from 'interface'

const servicosRepository = AppDataSource.getRepository(servicos)

class Servico {
    async criarServico(req: Request, res: Response, next: NextFunction) {
        try {
            const { servico_id, nomeServico, custo }: Servicos = req.body
    ...
    async deletarRelaçõesServiço(req: Request, res: Response, next: NextFunction) {
        try {
            const { id } = req.params
            await AppDataSource
                .createQueryBuilder()
    ...
    async deletarServico(req:Request, res:Response) {
        try {
            const {id} = req.params
            await servicosRepository.
    ...
    async editarServico(req:Request, res:Response){
        try {
            const {id} = req.params
            const body: Servicos = req.body
            await servicosRepository.
